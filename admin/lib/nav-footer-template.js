/**
 * Nav / Footer template — turns a structured model (menu items, footer columns,
 * text fields) into the exact <nav>/<footer> HTML the site expects. This lets
 * the admin panel offer a form-based editor instead of raw HTML editing.
 *
 * The structural shell (logo, search button, dropdown mechanics, mobile toggle,
 * social icons, license badge) is fixed here; only the editable content
 * (links, labels, text) is injected from the model.
 */
...
// Multi-line text -> paragraphs. Blank line separates <p>; single newline -> <br>.
function textToParagraphs(text, pClass) {
  const cls = pClass ? ` class="${pClass}"` : '';
  return String(text || '')
    .split(/\n\s*\n/)
    .map((block) => block.trim())
    .filter(Boolean)
    .map((block) => `<p${cls}>${block.split('\n').map((l) => esc(l.trim())).join('<br>')}</p>`)
    .join('\n          ');
}
...
function navItemHtml(item) {
  if (item && item.type === 'dropdown') {
    const subs = (item.children || [])
      .map((c) => `              <a href="${esc(c.url)}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-teal-700">${esc(c.label)}</a>`)
      .join('\n');
...
${subs}
  ...
  return `          <a href="${esc(item.url)}" data-nav-page="${esc(item.url)}" class="px-3 py-2 text-base font-medium text-gray-700 hover:text-teal-700 rounded-lg hover:bg-gray-50 transition-colors whitespace-nowrap">${esc(item.label)}</a>`;
}

function buildNavHtml(nav) {
  nav = nav || {};
  const items = (nav.items || []).map(navItemHtml).join('\n');
  const submitLabel = esc(nav.submitLabel || 'Submit Manuscript');
  const submitUrl = esc(nav.submitUrl || '#');
  return `<nav class="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm" aria-label="Main navigation">
...
function footerColumnHtml(col) {
  const links = (col.links || [])
    .map((l) => `            <li><a href="${esc(l.url)}" class="hover:text-white transition-colors">${esc(l.label)}</a></li>`)
    .join('\n');
  return `        <!-- ${esc(col.title)} -->
        <div>
          <h3 class="font-semibold text-lg mb-4">${esc(col.title)}</h3>
...
function buildFooterHtml(footer) {
  footer = footer || {};
  const columns = (footer.columns || []).map(footerColumnHtml).join('\n');
  const brandText = textToParagraphs(footer.brandText, 'text-teal-300 text-sm leading-relaxed');
  const contactText = textToParagraphs(footer.contactText);
  const email = esc(footer.contactEmail || '');
  return `<footer class="bg-teal-900 text-white" aria-label="Site footer">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div>
          <div class="flex items-center space-x-2 mb-4">
            ...
            <div>
              <div class="font-bold text-lg">${esc(footer.brandTitle || 'Balkan Medical Journal')}</div>
            </div>
          </div>
          ${brandText}
        </div>
...
${columns}
        ...
        <!-- Contact -->
        <div>
          <h3 class="font-semibold text-lg mb-4">${esc(footer.contactTitle || 'Contact')}</h3>
          <div class="text-teal-300 text-sm space-y-2">
          ${contactText}
          ${email ? `<p><a href="mailto:${email}" class="hover:text-white transition-colors">${email}</a></p>` : ''}
      ...
      <div class="border-t border-teal-800 mt-10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <p class="text-teal-400 text-sm">${esc(footer.copyright || '')}</p>
        <div class="flex items-center gap-6 text-teal-400 text-sm">
          <span>${esc(footer.licenseText || '')}</span>
...
// Default model — mirrors the current site nav/footer. Used to seed the editor.
const DEFAULT_MODEL = {
  nav: {
    items: [
      { type: 'link', label: 'Home', url: 'index.html' },
      { type: 'dropdown', label: 'About', children: [
        { label: 'About the Journal', url: 'about.html' },
        { label: 'Editorial Board', url: 'editorial-board.html' },
        { label: 'Journal Metrics', url: 'journal-metrics.html' },
      ] },
      { type: 'dropdown', label: 'Issues & Articles', children: [
        { label: 'Current Issue', url: 'current-issue.html' },
        { label: 'Articles in Press', url: 'articles-in-press.html' },
        { label: 'Archive', url: 'archive.html' },
      ] },
      { type: 'link', label: 'For Authors', url: 'for-authors.html' },
      { type: 'link', label: 'For Reviewers', url: 'for-reviewers.html' },
      { type: 'link', label: 'Policies', url: 'policies.html' },
  ...
  footer: {
    ...
    brandText: 'Published by Trakya University, Edirne, Türkiye.\nE-ISSN: 2146-3131\nPublisher: Galenos Publishing House',
    columns: [
      { title: 'Quick Links', links: [
        { label: 'Current Issue', url: 'current-issue.html' },
        { label: 'Archive', url: 'archive.html' },
        { label: 'Articles in Press', url: 'articles-in-press.html' },
        { label: 'For Authors', url: 'for-authors.html' },
        { label: 'For Reviewers', url: 'for-reviewers.html' },
      ...
      { title: 'About', links: [
        ...
        { label: 'Editorial Board', url: 'editorial-board.html' },
        { label: 'Journal Metrics', url: 'journal-metrics.html' },
        { label: 'Policies', url: 'policies.html' },
        { label: 'Contact', url: 'contact.html' },
        { label: 'News', url: 'news.html' },
      ] },
    ],
    contactTitle: 'Contact',
    contactText: 'Balkan Medical Journal\nTrakya University Faculty of Medicine\n\nBalkan Yerleşkesi, 22030\nEdirne, Türkiye',
    contactEmail: 'info@balkanmedicaljournal.org',
    copyright: '© 2026 Balkan Medical Journal. All rights reserved.',
    licenseText: 'Licensed under CC BY-NC-ND 4.0',
  },
};

// Build the full data record (model + generated HTML) from a model.
function buildNavFooterData(model) {
  const m = model && model.nav && model.footer ? model : DEFAULT_MODEL;
  return {
    _note: 'Form tabanlı düzenleyici ile yönetilir. nav/footer = yapısal model, navHtml/footerHtml = üretilen kod.',
    nav: m.nav,
    footer: m.footer,
    navHtml: buildNavHtml(m.nav),
    footerHtml: buildFooterHtml(m.footer),
  };
}

module.exports = { buildNavHtml, buildFooterHtml, buildNavFooterData, DEFAULT_MODEL };
